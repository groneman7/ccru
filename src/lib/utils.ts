...
export function combineDateAndTimeToUTC(localDate: Date, hhmm: string): Date | undefined {
    if (!hhmm) return undefined;
    const [hourStr, minuteStr] = hhmm.split(":");
    const hour = parseInt(hourStr!, 10);
    const minute = parseInt(minuteStr!, 10);

    // interpret localDate in America/New_York, set hour + minute, then convert to UTC
